...
df2 = pd.read_pickle('eo_rest_of_usa_concat') # import data frame with addresses
...
def get_latlong(row):
    ein = row['EIN']
    try:
        address =  row['STREET'] + ', '+ row['CITY'] + ', ' + row['STATE'] + ', ' + row['ZIP'] #[0:5]   
        result = client.street_address(address)
        location = result.location
        if row['STREET'][0:6] == 'PO BOX':
        ...
        with open('eo_rest_of_usa_concat_earlyexit.json', 'w') as fp:
            json.dump(geos, fp)
